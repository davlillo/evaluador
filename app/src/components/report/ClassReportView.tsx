              ...
              <tbody>
                {diagram.relationships.map((rel, i) => (
                  <tr key={i} className={i % 2 === 0 ? 'bg-background' : 'bg-muted/30'}>
                    ...
                    <td className="px-3 py-1.5 border font-mono text-center">{rel.source_multiplicity ?? '-'}</td>
                    <td className="px-3 py-1.5 border font-mono text-center">{rel.target_multiplicity ?? '-'}</td>
...
export function ClassReportView({ result, onBack }: ClassReportViewProps) {
  return (
